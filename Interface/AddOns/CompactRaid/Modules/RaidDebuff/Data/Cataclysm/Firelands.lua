------------------------------------------------------------
-- Firelands.lua
-- 作者 Abin
-- 更新 小渔(www.fishui.net)
-- 2012/09/07
------------------------------------------------------------

local module = CompactRaid:FindModule("RaidDebuff")
if not module then return end

local TIER = 4 -- Cataclysm
local INSTANCE = 78 -- Firelands

-- Beth'tilac (192)
module:RegisterDebuff(TIER, INSTANCE, 192, 97202) -- Fiery Web Spin
module:RegisterDebuff(TIER, INSTANCE, 192, 49026) -- Fixate
module:RegisterDebuff(TIER, INSTANCE, 192, 97079) -- Seeping Venom
module:RegisterDebuff(TIER, INSTANCE, 192, 99506, 5) -- The Widow's Kiss
module:RegisterDebuff(TIER, INSTANCE, 192, 100048) --Fiery Web Silk

-- Lord Rhyolith (193)
module:RegisterDebuff(TIER, INSTANCE, 193, 98492, 5) --Eruption

-- Alysrazor (194)
module:RegisterDebuff(TIER, INSTANCE, 194, 100744) -- Firestorm
module:RegisterDebuff(TIER, INSTANCE, 194, 101729) -- Blazing Claw
module:RegisterDebuff(TIER, INSTANCE, 194, 98734) -- Molten Feather
module:RegisterDebuff(TIER, INSTANCE, 194, 98619) -- Wings of Flame
--module:RegisterDebuff(TIER, INSTANCE, 194, 99461) -- Blazing Power
--module:RegisterDebuff(TIER, INSTANCE, 194, 100029) -- Alysra's Razor
module:RegisterDebuff(TIER, INSTANCE, 194, 99427) -- Incendiary Cloud
module:RegisterDebuff(TIER, INSTANCE, 194, 101484) -- Blazing Shield
module:RegisterDebuff(TIER, INSTANCE, 194, 100094, 5) --Fieroblast
module:RegisterDebuff(TIER, INSTANCE, 194, 99389) -- Imprinted
module:RegisterDebuff(TIER, INSTANCE, 194, 99308) -- Gushing Wound
module:RegisterDebuff(TIER, INSTANCE, 194, 100640) -- Harsh Winds
module:RegisterDebuff(TIER, INSTANCE, 194, 100555)--Smouldering Roots
module:RegisterDebuff(TIER, INSTANCE, 194, 99461, 5)--Blazing Power
module:RegisterDebuff(TIER, INSTANCE, 194, 99275, 5) --Molten Boulder

-- Shannox (195)
module:RegisterDebuff(TIER, INSTANCE, 195, 99837) -- Crystal Prison Trap Effect
module:RegisterDebuff(TIER, INSTANCE, 195, 52606) -- Immolation Trap
module:RegisterDebuff(TIER, INSTANCE, 195, 99936) -- Jagged Tear
module:RegisterDebuff(TIER, INSTANCE, 195, 99947) -- Face Rage
module:RegisterDebuff(TIER, INSTANCE, 195, 99840) -- Magma Rupture
-- Rageface
module:RegisterDebuff(TIER, INSTANCE, 195, 101208)--Immolation Trap
module:RegisterDebuff(TIER, INSTANCE, 195, 100415)--Rage
--module:RegisterDebuff(TIER, INSTANCE, 195, 52606) -- Immolation Trap

-- Baleroc, the Gatekeeper (196)
module:RegisterDebuff(TIER, INSTANCE, 196, 99252, 5) -- Blaze of Glory
module:RegisterDebuff(TIER, INSTANCE, 196, 99256, 5) -- Torment
module:RegisterDebuff(TIER, INSTANCE, 196, 99403) -- Tormented
module:RegisterDebuff(TIER, INSTANCE, 196, 99263, 5) -- Vital Flame
module:RegisterDebuff(TIER, INSTANCE, 196, 99262, 5) -- Vital Spark
module:RegisterDebuff(TIER, INSTANCE, 196, 99516) -- Countdown
module:RegisterDebuff(TIER, INSTANCE, 196, 99353) -- Decimating Strike
module:RegisterDebuff(TIER, INSTANCE, 196, 100908, 5) -- Fiery Torment

-- Majordomo Staghelm (197)
module:RegisterDebuff(TIER, INSTANCE, 197, 98535, 5) -- Leaping Flames
module:RegisterDebuff(TIER, INSTANCE, 197, 98443) -- Fiery Cyclone
module:RegisterDebuff(TIER, INSTANCE, 197, 98450) -- Searing Seeds
--Burning Orbs
module:RegisterDebuff(TIER, INSTANCE, 197, 100210, 5) -- Burning Orb
module:RegisterDebuff(TIER, INSTANCE, 197, 96993) -- Stay Withdrawn

-- Ragnaros (198)
module:RegisterDebuff(TIER, INSTANCE, 198, 100293) -- Lava Wave
module:RegisterDebuff(TIER, INSTANCE, 198, 98313, 5) -- Magma Blast
module:RegisterDebuff(TIER, INSTANCE, 198, 100757) -- Deluge
module:RegisterDebuff(TIER, INSTANCE, 198, 99399, 5) -- Burning Wound
module:RegisterDebuff(TIER, INSTANCE, 198, 100238, 5) -- Magma Trap Vulnerability
--Heroic Phase 4
module:RegisterDebuff(TIER, INSTANCE, 198, 100941) --Dreadflame
module:RegisterDebuff(TIER, INSTANCE, 198, 100594, 5) --Superheated
module:RegisterDebuff(TIER, INSTANCE, 198, 100460) -- Blazing Heat
module:RegisterDebuff(TIER, INSTANCE, 198, 98981) -- Lava Bolt
module:RegisterDebuff(TIER, INSTANCE, 198, 100249) -- Combustion
module:RegisterDebuff(TIER, INSTANCE, 198, 99613) -- Molten Blast
--module:RegisterDebuff(TIER, INSTANCE, 198, 100757) -- Deluge

-- Trash
module:RegisterDebuff(TIER, INSTANCE, 0, 76622, 5) -- Sunder Armor
module:RegisterDebuff(TIER, INSTANCE, 0, 99610) -- Shockwave
module:RegisterDebuff(TIER, INSTANCE, 0, 99695) -- Flaming Spear
module:RegisterDebuff(TIER, INSTANCE, 0, 99800) -- Ensnare
module:RegisterDebuff(TIER, INSTANCE, 0, 99625)--Conflagration (Magic/dispellable)
--module:RegisterDebuff(TIER, INSTANCE, 0, 16856)--Mortal Strike
module:RegisterDebuff(TIER, INSTANCE, 0, 99993, 5) -- Fiery Blood
module:RegisterDebuff(TIER, INSTANCE, 0, 100767, 5) -- Melt Armor
--module:RegisterDebuff(TIER, INSTANCE, 0, 99692)--Terrifying Roar (Magic/dispellable)
module:RegisterDebuff(TIER, INSTANCE, 0, 99693) -- Dinner Time
module:RegisterDebuff(TIER, INSTANCE, 0, 100549, 5) -- Lava Surge
module:RegisterDebuff(TIER, INSTANCE, 0, 100057, 5) -- Rend Flesh
module:RegisterDebuff(TIER, INSTANCE, 0, 99762, 5) --Flames of the Firehawk
module:RegisterDebuff(TIER, INSTANCE, 0, 101166) -- Ignite
module:RegisterDebuff(TIER, INSTANCE, 0, 100526, 5) -- Blistering Wound
module:RegisterDebuff(TIER, INSTANCE, 0, 100095) -- Fieroclast Barrage
module:RegisterDebuff(TIER, INSTANCE, 0, 99646) --Reckless Leap (Magic/dispellable)
module:RegisterDebuff(TIER, INSTANCE, 0, 99626) --Sunfire (Magic/dispellable)
module:RegisterDebuff(TIER, INSTANCE, 0, 99650, 5) -- Reactive flames
module:RegisterDebuff(TIER, INSTANCE, 0, 97151, 5) -- Magma
module:RegisterDebuff(TIER, INSTANCE, 0, 100274, 5) -- Blessed Defender of Nordrassil
module:RegisterDebuff(TIER, INSTANCE, 0, 100324) --Molten Fixation
