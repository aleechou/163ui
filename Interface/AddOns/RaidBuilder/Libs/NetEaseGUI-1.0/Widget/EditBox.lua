...
function EditBox:Constructor(parent)
	...
	local prompt = CreateFrame('Button', nil, editBox)
	prompt:SetPoint('TOPLEFT', 3, 0)
	prompt:EnableMouse(false)
	local promptFS = prompt:CreateFontString(nil, 'BORDER', 'GameFontHighlightSmall')
	promptFS:SetJustifyH('LEFT')
	promptFS:SetJustifyV('MIDDLE')
	promptFS:SetTextColor(0.35, 0.35, 0.35)
	prompt:SetFontString(promptFS)
	prompt:Hide()
