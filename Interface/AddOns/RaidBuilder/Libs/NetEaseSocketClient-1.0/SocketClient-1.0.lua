...
local MAJOR, MINOR = 'SocketClient-1.0', 11
...
function SocketClient:IsLoggedIn()
    local self = SocketClient
    ...
    if IsLoggedIn() then
        if not self.waitForLogin then
            self:ScheduleTimer('PLAYER_LOGIN', CONNECT_DELAY)
        end
    else
        AceEvent.RegisterEvent(self, 'PLAYER_LOGIN', function()
            self:ScheduleTimer('PLAYER_LOGIN', CONNECT_DELAY)
            AceEvent.UnregisterEvent(self, 'PLAYER_LOGIN')
        end)
        self.waitForLogin = true
    end
end
