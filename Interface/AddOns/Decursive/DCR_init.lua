...
local function InitVariables_Once() -- {{{
    ...
    D.Revision = "b9157b8"; -- not used here but some other add-on may request it from outside
    D.date = "2014-10-15T11:38:23Z";
    D.version = "2.7.3.5";
...
function D:VersionWarnings(forceDisplay) -- {{{
    ...
    if DcrMaxTOC < T._tocversion then
        ...
        elseif time() - self.db.global.TocExpiredDetection > 3600 * 24 * 40 then -- if more than 40 days elapsed since the detection
            ...
            if time() - self.db.global.LastExpirationAlert > 48 * 3600 or forceDisplay then
                ...
                T._ShowNotice ("|cff00ff00Decursive version: 2.7.3.5|r\n\n" .. "|cFFFFAA66" .. L["TOC_VERSION_EXPIRED"] .. "|r");
    ...
    if (("2.7.3.5"):lower()):find("beta") or ("2.7.3.5"):find("RC") or ("2.7.3.5"):find("Candidate") or alpha then
        ...
        if self.db.global.NonRelease ~= "2.7.3.5" then
            self.db.global.NonRelease = "2.7.3.5";
...
T._LoadedFiles["DCR_init.lua"] = "2.7.3.5";
...
b9157b8989bedcca45bf8187b7f6fd04ee90309b
