...
function mod:SPELL_SUMMON(args)
	if args:IsSpellID(63811) then -- Bomb Bot
		warnBombSpawn:Show()
		sndWOP:Play("Interface\\AddOns\\DBM-Core\\extrasounds\\"..DBM.Options.CountdownVoice.."\\bombbot.mp3")
	end
end
...
function mod:SPELL_CAST_START(args)
	if args:IsSpellID(63631) then
		...
		if self.Options.PlaySoundOnShockBlast then
			PlaySoundFile("Interface\\AddOns\\DBM-Core\\extrasounds\\"..DBM.Options.CountdownVoice.."\\boomrun.mp3")
		end
	end
	if args:IsSpellID(64529, 62997) then -- plasma blast
		timerPlasmaBlastCD:Start()
		if mod:IsTank() or mod:IsHealer() then
			sndWOP:Play("Interface\\AddOns\\DBM-Core\\extrasounds\\"..DBM.Options.CountdownVoice.."\\plasma.mp3")
		end
	end
	...
	if args:IsSpellID(64623) then
		warnFrostBomb:Show()
		sndWOP:Play("Interface\\AddOns\\DBM-Core\\extrasounds\\"..DBM.Options.CountdownVoice.."\\icebomb.mp3")
...
local function show_warning_for_spinup()
	if is_spinningUp then
		...
		if mod.Options.PlaySoundOnDarkGlare then
			PlaySoundFile("Interface\\AddOns\\DBM-Core\\extrasounds\\"..DBM.Options.CountdownVoice.."\\spinrun.mp3")
		end
	end
end
