...
local Addon = LibStub("AceAddon-3.0"):GetAddon("RuneFrame")
...
local function createBottom()
	...
	runeFrameParent = CreateFrame("Frame","RuneFrame",UIParent)
	...
	artwork:SetTexture([[Interface\AddOns\RuneFrame\Media\bg]])
	...
	bottomBar:SetStatusBarTexture([[Interface\AddOns\RuneFrame\Media\bar]])
	...
	runeBorder.texture:SetTexture([[Interface\AddOns\RuneFrame\Media\iconbox]])
	...
	-- 开关按钮
	header = CreateFrame("Button","RuneHeader",runeFrameParent)
	...
	--符能
	powerText = runeFrameParent:CreateFontString("RunePowerText","OVERLAY","NumberFont_Outline_Med")
...
function A:OnInitialize()
	...
	self.db = LibStub("AceDB-3.0"):New("RuneFrame_Config",default)
	db = self.db.profile
	createBottom()
