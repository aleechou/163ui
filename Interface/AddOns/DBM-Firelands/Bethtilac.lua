...
function mod:repeatDrone()
	timerDrone:Start()
	sndWOP:Schedule(60, "Interface\\AddOns\\"..DBM.Options.CountdownVoice.."\\drone.mp3")
...
function mod:OnCombatStart(delay)
	...
	sndWOP:Schedule(45-delay, "Interface\\AddOns\\"..DBM.Options.CountdownVoice.."\\drone.mp3")
...
function mod:SPELL_AURA_APPLIED(args)
	...
	elseif args:IsSpellID(99526, 99559) and args:IsDestTypePlayer() then--99526 is on player, 99559 is on drone, leaving both for now with a filter, may remove 99559 and filter later.
		...
		if args:IsPlayer() then
			specWarnFixate:Show()
			sndWOP:Play("Interface\\AddOns\\"..DBM.Options.CountdownVoice.."\\awayspider.mp3")
		end
	end
end
...
function mod:SPELL_CAST_START(args)
	if args:IsSpellID(99052) then
		...
		if smolderingCount == 3 then	-- 3rd cast = start P2
			...
			timerDrone:Cancel()
			sndWOP:Cancel("Interface\\AddOns\\"..DBM.Options.CountdownVoice.."\\drone.mp3")
			...
			if smolderingCount == 2 then
				sndWOP:Schedule(80, "Interface\\AddOns\\"..DBM.Options.CountdownVoice.."\\ptwo.mp3")
			end
		end
	end
end
...
function mod:RAID_BOSS_EMOTE(msg)
	if msg == L.EmoteSpiderlings then
		sndWOP:Play("Interface\\AddOns\\"..DBM.Options.CountdownVoice.."\\spiderling.mp3")
		self:UnscheduleMethod("repeatSpiderlings")	-- in case it is off
		self:repeatSpiderlings()
	elseif msg == L.EmoteSpinners then
		sndWOP:Schedule(1, "Interface\\AddOns\\"..DBM.Options.CountdownVoice.."\\spinner.mp3")
		sndWOP:Schedule(11, "Interface\\AddOns\\"..DBM.Options.CountdownVoice.."\\spinner.mp3")
		sndWOP:Schedule(21, "Interface\\AddOns\\"..DBM.Options.CountdownVoice.."\\spinner.mp3")
	end
end
