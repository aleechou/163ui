...
EA_Config = { DoAlertSound, AlertSound, AlertSoundValue, LockFrame, ShareSettings, ShowFrame, ShowName, 
	...
	AllowAltAlerts, Target_MyDebuff,IsUseFloat };
...
function EventAlert_OnEvent(self, event, ...)
	...
	if (event == "ADDON_LOADED") then
		...
		if (arg1 == "EventAlertMod") then
			...
			if EA_Config.IsUseFloat == nil then EA_Config.IsUseFloat = false end;		--buff,debuff,cd的剩餘時間是否顯示到小數點第一位(true/false)
			--EA_Config.IsUseFloat=true;							--強迫啟用
...
function EventAlert_OnSCDUpdate(spellId)
	...
	local EA_start, EA_duration, EA_Enable = GetSpellCooldown(spellId);								--取得技能冷卻
	local EA_ChargeCurrent, EA_ChargeMax, EA_ChargeStart,EA_ChargeDuration = GetSpellCharges(spellId);				--取得技能充能
		...
		if EA_ChargeCurrent then
			...
			if (EA_Enable ~= 0) then
				if (EA_start > 0) and (EA_duration > 0)  then
					...
					if (EA_timeLeft > 1.5) then
						ActionButton_HideOverlayGlow(eaf)
							
						if (EA_Config.ShowTimer) then EAFun_SetCountdownStackText(eaf, EA_timeLeft+0.5, 0, -1);	end				
...
function EventAlert_ShowVerURL(SiteIndex)
	...
	-- WOW API禁止直接啟動瀏覽器連到網頁，只能顯示URL在對話視窗中。
...
function EventAlert_PlayerSpecPower_Update()
	...
	elseif (pClass == EA_CLASS_DRUID) then
		--if (shapeindex == 1)	then 	-- 熊D(1)
		if (shapeID == 5)	then 	-- 熊D(5), 貓D(1)
			EA_SpecHasPower.HasRage = true;			
		end
	end
	...
	elseif (pClass == EA_CLASS_DRUID) then
		if ((shapeID == 5) or (shapeID == 1)) then 	-- 熊D(5), 貓D(1)
			EA_SpecHasPower.HasEnergy = true;
		end
