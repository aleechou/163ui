...
function mod:SPELL_AURA_APPLIED(args)
	if args:IsSpellID(123707) then
		...
		if args:IsPlayer() and (args.amount or 1) >= 4 then
			...
			sndWOP:Play(DBM.SoundMMPath.."\\ex_mop_csgg.ogg") --層數過高
		else
	...
	elseif args:IsSpellID(123788) then
		...
		if args:IsPlayer() then
			...
			sndWOP:Play(DBM.SoundMMPath.."\\ex_mop_kjyy.ogg") --快進音域
		end
	...
	elseif args:IsSpellID(124862) then
		...
		if self:AntiSpam(5, 3) then
			...
			if MyTT() then
				...
				sndWOP:Schedule(1, DBM.SoundMMPath.."\\ex_mop_zlzb.ogg") --戰慄準備
				sndWOP:Schedule(1.5, DBM.SoundMMPath.."\\countthree.ogg")
				sndWOP:Schedule(2.5, DBM.SoundMMPath.."\\counttwo.ogg")
				sndWOP:Schedule(3.5, DBM.SoundMMPath.."\\countone.ogg")
			end
		...
		if mod:IsHealer() then
			sndWOP:Schedule(4, DBM.SoundMMPath.."\\dispelnow.ogg")
		end
	...
	elseif args:IsSpellID(124097) then
		if args:IsPlayer() and self:AntiSpam(5, 2) then --prevent spam in heroic
			...
			sndWOP:Play(DBM.SoundMMPath.."\\ex_mop_szkp.ogg") --樹脂
		end
	...
	elseif args:IsSpellID(123845) then
		...
		if args:IsPlayer() then
			...
			sndWOP:Play(DBM.SoundMMPath.."\\ex_mop_lxkp.ogg")--連線快跑
		end
	end
end
...
function mod:SPELL_CAST_SUCCESS(args)
	...
	elseif args:IsSpellID(125826) then
		...
		sndWOP:Play(DBM.SoundMMPath.."\\ex_mop_xjwc.ogg") --陷阱完成
...
function mod:SPELL_CAST_START(args)
	if args:IsSpellID(124849) then
		...
		sndWOP:Play(DBM.SoundMMPath.."\\ex_mop_kjts.ogg")--恐懼吞噬
	end
end
...
function mod:UNIT_SPELLCAST_SUCCEEDED(uId, _, _, _, spellId)
	if spellId == 125098 then--Yell is about 1.5 seconds faster then this event, BUT, it also requires localizing. I don't think doing it this way hurts anything.
		...
		sndWOP:Schedule(152, DBM.SoundMMPath.."\\ex_mop_nwzb.ogg") --女王準備
		sndWOP:Schedule(153, DBM.SoundMMPath.."\\countfour.ogg")
		sndWOP:Schedule(154, DBM.SoundMMPath.."\\countthree.ogg")
		sndWOP:Schedule(155, DBM.SoundMMPath.."\\counttwo.ogg")
		sndWOP:Schedule(156, DBM.SoundMMPath.."\\countone.ogg")
		if self.Options.InfoFrame then
	...
	elseif spellId == 125304 then
		...
		sndWOP:Cancel(DBM.SoundMMPath.."\\ex_mop_nwzb.ogg")
		...
		ptwo = false
		sndWOP:Play(DBM.SoundMMPath.."\\phasechange.ogg")--階段轉換
		sndWOP:Schedule(147, DBM.SoundMMPath.."\\ptwo.ogg")--P2
		sndWOP:Schedule(148, DBM.SoundMMPath.."\\countfour.ogg")
		sndWOP:Schedule(149, DBM.SoundMMPath.."\\countthree.ogg")
		sndWOP:Schedule(150, DBM.SoundMMPath.."\\counttwo.ogg")
		sndWOP:Schedule(151, DBM.SoundMMPath.."\\countone.ogg")
...
function mod:RAID_BOSS_EMOTE(msg)
	if msg:find("spell:126121") then
		sndWOP:Play(DBM.SoundMMPath.."\\ex_mop_yyfh.ogg") --音域腐化
	end
end