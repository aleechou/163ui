...
local statusToModuleMap = {} --warbaby 用来记录status属于哪个模块
function GridStatus:IsStatusEnabled(status)
	return statusToModuleMap[status].db.profile[status].enable
end
function GridStatus:GetStatusToModuleMap() return statusToModuleMap end
...
function GridStatus.modulePrototype:RegisterStatus(status, desc, options, inMainMenu, order)
	...
	local module = self
	statusToModuleMap[status] = module
	if not optionMenu[status] then
		optionMenu[status] = {
			name = desc,
			desc = format(L["Status: %s"], desc),
			order = status=="unit_name" and 0 or (inMainMenu and 111 or order),
			type = "group",
			args = {
				["enable"] = {
					name = L["Enable"],
