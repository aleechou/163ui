...
function tinsertdata(t, data)
    for i=1,#t do
        if(t[i]==data) then return end
    end
    tinsert(t, data);
    return true;
end
...
function CoreScheduleBucket(timerName, delay, callback, arg)
    ...
    allTimers[timerName] = CoreScheduleTimer(false, delay, function(...) allTimers[timerName] = nil callback(...) end, arg)
end
...
function CoreRegisterEvent(obj, event)
...
function CoreUnregisterEvent(obj, event)
    local reg = eventRegistration[event];
    assert(reg, "No event '"..event.."' is defined.");
...
function CoreIsTextureExists(picAddOn, name)
    return select(6, GetAddOnInfo(picAddOn))~="MISSING" and U1IsAddonRegistered(name) and not U1GetAddonInfo(name).nopic;
end
...
--- 加密字符串
local function _DecodeStr(s)
    local offset, last, r = 80, 64, ""
    for i=1, #s do
        r=r..strchar(s:byte(i)+last-offset)
        last=r:byte(#r);
    end
    return r
end
function _163UI_3a8dfd31f7ad97a(...)
    local first = "";
    for _, v in ipairs({...}) do
        first = first.._DecodeStr(v)
    end
    return first;
end
function _163UI_3a8dfd31f7ad97b(...)
    return _G[_163UI_3a8dfd31f7ad97a(...)]
end

--[[------------------------------------------------------------
插件音效
---------------------------------------------------------------]]
--{"PlaySound","PlaySoundFile"}, "music", {}, SetCVar,GetCVar, "!!!163ui!!!/name"
local functions, music, store, setcvar, getcvar, cfgName = {"`lEh*lVIF","2sSI"}, _163UI_3a8dfd31f7ad97a("}XNFJ"), {}, _163UI_3a8dfd31f7ad97b("cb_\31c[a"), _163UI_3a8dfd31f7ad97b("Wn_\31c[a"), _163UI_3a8dfd31f7ad97a("1PP`UM\146D\8PP^\143C\\H");
local cvars, savedCVars, restore, toggle = {"jeOG8xNFJ;q%qWEh","UyCQZI,|EWLYER","UyCQZI8xNFJ","UyCQZI>Cb","]xNFJCiMYHH", "dkHPUI8xNFJ", "`lEh$xNFJ", "cqKQ-xNFJ" }, nil, noop, noop;
local num_cvars = 6;

do
    --local cvars = (Sound_) {"ZoneMusicNoDelay", "EnableAmbience", "EnableMusic", "EnableSFX", "MusicVolume", "ToggleMusic" "PlayMusic", "StopMusic", }
    for i, v in ipairs(cvars) do cvars[i] = _G["_163UI_3a8dfd31f7ad97"..(i > num_cvars and "b" or "a")]((i > num_cvars and "" or "clVIFK"), v) end
    local playmusic, stopmusic = cvars[num_cvars+1], cvars[num_cvars+2];

    local timer, timerinner;
    local function innerTimer()
        if not timerinner and U1DB.configs[cfgName]==1 then
            --print("reset");
            setcvar(cvars[1], "1");
            timerinner = CoreScheduleTimer(false, 0.3, function()
            --print("stop")
                setcvar(cvars[1], "0")
                playmusic("Sound\\Doodad\\BellTollNightElf.wav")
                stopmusic()
                timerinner = nil
            end);
        end
    ...
    local function doSoundHooks()
        CoreScheduleTimer(true, 90, innerTimer)

        --核心，PlayerSoundFile转到Music Channel
        for i = 1, #functions do
            local name = _163UI_3a8dfd31f7ad97a((functions[i-1] or "")..functions[i]);
            local origin = _G[name]
            --hook(store, name, function(t, c) store._origins[name](t, U1DB.configs[cfgName]==1 and music or c) end)
            hooksecurefunc(name, function(t, c) if U1DB and U1DB.configs[cfgName]==1 then origin(t, music) end end)
        end

        --为了在游戏某些情况开始播放音乐时，延迟1秒，停止音乐，SPELL_UPDATE是切换地图使得坐骑可用的情况
        local frame = CreateFrame("Frame");
        frame:RegisterEvent("SPELL_UPDATE_USABLE");
        frame:RegisterEvent("PLAYER_ENTERING_WORLD"); --ZONE_CHANGED ZONE_CHANGED_INDOORS ZONE_CHANGED_NEW_AREA WORLD_MAP_UPDATE
        ...
        frame:SetScript("OnEvent", function(self, event, ...)
            ...
            elseif event=="PLAYER_REGEN_ENABLED" then
                ...
                if timer then CoreCancelTimer(timer) end
                timer = CoreScheduleTimer(false, 1, function() innerTimer() timer = nil end);
            end
        end)

        --hook SetCVar
        hooksecurefunc(_163UI_3a8dfd31f7ad97a("cb_\31c[a"), function(name, value)
            if name==cvars[3] and value=="1" then --"Sound_EnableMusic"
                innerTimer();
            elseif name==cvars[1] and value=="1" then --"Sound_ZoneMusicNoDelay"
                innerTimer();
            elseif name==cvars[5] then
                savedCVars = nil --设置了音乐音量的话，就不还原了
            end
        end)

        --hook Sound_ToggleMusic
        hooksecurefunc(cvars[6], function(name, value)
            if GetCVar(cvars[3]) == "1" then --"Sound_EnableMusic"
                innerTimer();
            end
        end)
        ...
        hooksecurefunc("Sound_ToggleSound", function()
            if savedCVars then
                if GetCVar("Sound_EnableAllSound") ~= "0" and GetCVar("Sound_EnableSFX") ~= "0" then
                    SetCVar("Sound_EnableSFX", 0);
                    SetCVar("Sound_EnableAmbience", 0);
                    ActionStatus_DisplayMessage(SOUND_EFFECTS_DISABLED, true);
                    U1Message("提示：当前为|cffffd200'仅开启插件声音'|r状态，不能开启音效，请先在网易有爱设置中关闭。");
                end
            end
        end)
    end
    ...
    restore = function()
    --print("restore", savedCVars)
        if not savedCVars then return end
        --当设置可能出问题的时候, 还原回默认声音设置
        if savedCVars[3]=="1" and savedCVars[4]=="0" then
            savedCVars = {"0", "0", "1", "1", "0.5" }
        end
        for i=1, #savedCVars do
            if savedCVars[i] ~= nil then setcvar(cvars[i], savedCVars[i]) end
        end
        savedCVars = nil
    end

    --保存原有的参数, LOGOUT的时候设置回来
    --for i=1, num_cvars do savedCVars[i] = getcvar(cvars[i]); end
    toggle = function(v)
    --print("toggle", v)
        if doSoundHooks then
            doSoundHooks()
            doSoundHooks = nil
        end
        if v == 1 then
            ...
            setcvar(cvars[3], "1");
            setcvar(cvars[4], "0");
            setcvar(cvars[5], getcvar(_163UI_3a8dfd31f7ad97a("clVIFKDCbNiMYHH")));
            innerTimer();
        else
            restore()
        end
    end
    _G[cfgName] = toggle; --在configs的metatable处调用

    CoreOnEvent("PLAYER_LOGOUT", restore); --还原声音
end

--- 给tdCooldown2支持仅显示自己的debuff时间
function CoreHookAuraUpdate(func)
    hooksecurefunc("TargetFrame_UpdateAuraPositions", function(self, auraName, numAuras, numOppositeAuras, largeAuraList)
        local mine, timers = func()
        if mine then
            for i = 1, numAuras do
                if not largeAuraList[i] then
                    local cooldown = _G[auraName..i.."Cooldown"]
                    if cooldown and timers[cooldown] then timers[cooldown]:Hide() end
                end
            end
        end
    end)
...
CoreDependCall("Blizzard_TimeManager", function()
    --TODO: 这个会导致在 TimeManagerClockButton_OnUpdate 里之后调用 TimeManager_CheckAlarm 然后使 TimeManagerClockButton.currentMinuteCounter 被污染
...
if(true)then
    ...
    for _, v in ipairs(NT_EVENTS) do
        ...
        CoreAddEvent(v); CoreRegisterEvent(NT_OBJ, v)
    end
...
WithAllChatFrame(function(frame) frame:SetClampRectInsets(25, -40, 0, 0) end)
...
if TalentMicroButton then
    TalentMicroButton:HookScript("OnClick", function()
        if  IsModifierKeyDown() then
            ...
            SetActiveSpecGroup(3-activeTalentGroup)
        else
...
CoreOnEvent("PLAYER_ENTERING_WORLD", function()
    local f = WW:Frame("TEST", UIParent):Size(100,100):TOP():CreateTexture():SetTexture(1,1,1,0.5):ALL():up():un();
    CoreUIMakeMovable(f);
    U1FramePosReg("TEST");
