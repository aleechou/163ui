...
API：
 ...
 - wrapped = WW:Frame(...), WW:Button(...) 调用CreateFrame("Frame"/"Button", ...)的快捷方法, 返回的对象是已封装的
 - wrapped = WW:Font(...)
...
wrapper_meta.__index = function(wrapper, key)
    ...
    --处理CreateAnimation, CreateAnimationGroup, CreateFontString, CreateTexture, CreateTitleRegion, 返回结果同样Wrapper
    if key=="CreateAnimation" or key=="CreateAnimationGroup" or key=="CreateFontString" or key=="CreateTexture" or key=="CreateTitleRegion" or key=="CreateControlPoint" then
        func = function(self, ...)
...
-- 方便在WOWLua里测试的
-- W:Run(function(name, first) W:Frame(name); if first then ... end end)
