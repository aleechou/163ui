...
function BF:PLAYER_DEAD()
    if(U1GetCfgValue('!!!163ui!!!', 'bfautorelease')) then
        return RepopMe()
    end
end
...
local has_mapster = select(6, GetAddOnInfo'mapster') ~= 'MISSING'
...
function RaidColor:Init()
    if(U1GetCfgValue('!!!163ui!!!', 'map_raid_color')) then
        return self:Enable()
    else
        return self:Disable()
    end
