...
function P:EnableOrDisableAddOn(addons)
    if(addons) then
        for k, v in pairs(addons) do
            local origin = select(4, GetAddOnInfo(k))
            if v==1 then
...
function P:LoggingOut()
    if(UnitLevel("player")>=10) then
        self:BackupSession(L["登出"])
    end
end
