...
UUI.DropDownItems = QuickMenus or {
    ...
    "!!!163ui!!!/minimap_ping",
    "!!!163ui!!!/changeClip",
    "!!!163ui!!!/hideCompactRaid",
    "!!!163ui!!!/profanityFilter",
    "!!tradeskill/knownRecipes,1",
    "tinytip/tekcompare,1",
    "tdcooldown2/coolline/lock",
    --"163ui_chat/clean",
    "dbm-core/range",
    "163ui_bossguide/",
}
...
UUI.DEFAULT_TAG = ((DEBUG_MODE or UI163_USE_X_CATEGORIES) and "ALL") or "NETEASE"

UUI.FONT_PANEL_BUTTON = "U1FPanelButtonHei"
WW:Font3(UUI.FONT_PANEL_BUTTON, ChatFontNormal, 14.1, {{1,.82,0},{1,1,1},{.5,.5,.5}}, nil, 1, -1)
WW:Font3("U1FBannerHei", ChatFontNormal, 16, {{.91,.72,0},{1,1,1},{.5,.5,.5}}, nil, 1, -1)
WW:Font3("U1FCenterHei", ChatFontNormal, 14.1, {{0.81, 0.65, 0.48},{1,1,1},{.5,.5,.5}}, nil, 1, -1)
WW:Font("U1FCenterTextMid", ChatFontNormal, 14.1):SetFontFlags():SetShadowOffset(2,-2):un();
WW:Font("U1FCenterTextTiny", ChatFontNormal, 11):SetShadowOffset(1,-2):un();
...
function UUI.Tex(name)
    return "Interface\\AddOns\\!!!163UI!!!\\Textures\\"..name
end
...
function UUI.TransCfgToDropDown(path, info)
    ...
    if select(6, GetAddOnInfo(addon))=="MISSING" and not flagAlways then return end
    if path=="" then
        ...
        if (not U1IsAddonInstalled(addon) and not flagAlways) or not U1IsAddonRegistered(addon) then return end
...
function UUI.Raise(raise)
    ...
    if(raise~=false)then
        ...
        if(U1ProfileFrame and U1ProfileFrame:IsVisible()) then U1ProfileFrame:Raise() end
    else
...
function UUI.SetAddonTooltip(addonName, tip)
    ...
    tip:AddLine(info.parent and L["子插件"] or info.title or info.name);
    ...
    if(reasonInfo) then
        ...
        if(depNum > 0) then
            for i=1, depNum do
                ...
                local _, _, _, depEnabled, _, depReason = GetAddOnInfo(depName)
    ...
    --if(info.version)then tip:AddLine(UUI.formatTip("版本", info.version), 1, 1, 1, true); end
...
function UUI.Left.Create(main)
    ...
    left:Button():Key("btn163"):Size(128,32):TL(-14,-6):Set3Fonts("U1FBannerHei"):SetText(L["　有爱整合　"]):Texture(nil, nil, UUI.Tex'UI2-banner', 0,1,0,0.5):ToTexture("Normal"):ALL():up():un()
...
function UUI.Left.ScrollHybridCreator(self, index, name)
    local btn = WW:Button(nil, self.scrollChild):Size(104, 32):Set3Fonts("U1FCenterHei"):SetText(L["地图任务"])
...
function UUI.Top.Create(main)
    --左上角LOGO及文字
    main:CreateTexture():Key("logo"):SetTexture(UUI.Tex"UI2-logo"):TL(-18, 38):Size(128):un()
    ...
    --还原按钮
    main.btnUp = main:Button():Size(main.btnClose:GetSize()):TR(main.btnClose, -2, 0):Hide()
    :SetNormalTexture([[Interface\Buttons\UI-Panel-BiggerButton-Up]])
    :SetPushedTexture([[Interface\Buttons\UI-Panel-BiggerButton-Down]])
    :SetHighlightTexture([[Interface\Buttons\UI-Panel-MinimizeButton-Highlight]], "ADD")
    :SetScript("OnClick", UUI.CloseCompactPanel)
    :un()
    CoreUIEnableTooltip(main.btnUp, L["正常模式"], L["将界面还原成普通模式，而不是半透明的精简模式"]);
    ...
    local soundSlider = TplSlider(main.setting.soundPanel, nil, 1, "%d%%"):Size(12,128):TOP(-10, -35)
    :SetScript("OnEnter", UIDropDownMenu_StopCounting):SetScript("OnLeave", UIDropDownMenu_StartCounting)
    ...
    UUI.AddChangeWithColsButton(main.collect, L["回收内存"], L["内存"])

    main.profile = TplPanelButton(main,nil, UUI.PANEL_BUTTON_HEIGHT):Set3Fonts(UUI.FONT_PANEL_BUTTON)
    :SetText(L["方案管理"])
    :SetScript("OnClick", function(self) if not U1ProfileFrame then U1Profiles:CreateFrame() else CoreUIShowOrHide(U1ProfileFrame, not U1ProfileFrame:IsVisible()) end U1ProfileFrame:Raise(); UUI.Raise(not U1ProfileFrame:IsVisible()) end)
...
function UUI.Top.ReloadOnEnter(self)
    ...
    if(next(reloadList))then
        ...
        for k,v in pairs(reloadList) do
            ...
            local info = U1GetAddonInfo(name);
            while info.parent do info = U1GetAddonInfo(info.parent) type = "sub" end --子插件停用显示父插件配置
            if not tmp[info.name:lower()] then
                local title = U1GetAddonTitle(info.name);
                GameTooltip:AddLine( (type=="__disable" and L["|cffff0000停用|r - "] or L["配置 - "])..title)
                tmp[info.name:lower()] = 1
                i=i+1
...
function UUI.Center.Create(main)
    ...
    center:Frame():CreateTexture(nil, "OVERLAY"):SetTexture(UUI.Tex'UI2-shade-dark-deeper'):ALL():up():SetHeight(64):BL(l,"TL"):BR(r,"TR"):AddFrameLevel(2, center)
...
function UUI.Center.ScrollCreateOnButton(lineButton)
    ...
    local check = b:CheckButton(nil, "UICheckButtonTemplate"):Key("check"):RIGHT(-8, 0):Size(UUI.CHECK_W):AddFrameLevel(1)
    ...
    b:CreateFontString():Key("text1"):LEFT(UUI.ICON_W + 11, 1):RIGHT(check, "LEFT", -1, 0):SetJustifyH("LEFT")
    --:SetFontObject("U1FCenterTextMid"):SetText("网易有爱中心") --for demo
    :un()

    b:CreateFontString():Key("text2"):LEFT(UUI.ICON_W + 11, -9):RIGHT(check, "LEFT", -1, 0):SetJustifyH("LEFT"):SetFontObject(U1FCenterTextTiny):un()
...
function UUI.Center.ScrollUpdateOneButton(b, idx)
    ...
    b.text1:SetFontObject( (not info.registered or U1GetShowOrigin()) and U1FCenterTextTiny or U1FCenterTextMid)
    ...
    b.text1:SetPoint("LEFT", UUI.ICON_W + 11, showText2 and 6 or 1)
...
function UUI.Center.BtnLoadAllOnClick(self)
    ...
    UUI.loadTimer = CoreScheduleTimer(true, 0.3, UUI.LoadAddons, deepToggleChildren);
end
...
function UUI.Right.Create(main)
    ...
    local check = CoreUICreateCheckButtonWithIcon(right, nil, 22, 16):Key("check"):Set3Fonts(UUI.FONT_PANEL_BUTTON):BL(right, "TL", -3,7):un();
    ...
    :CreateTexture(nil,"BACKGROUND"):Key("wm"):SetTexture(UUI.Tex'UI2-watermark'):SetAlpha(0.125):Size(256, 64):BL(5, 0):up()
    :un();
    --WW(scroll):CreateTexture():SetTexture(1,1,1):ALL()
...
function UUI.Right.CreatePageDesc(right)
    ...
    :SetFont("P",font,13):SetTextColor("P",0.81, 0.65, 0.48):SetSpacing("P",5) --cfa67f
    :SetFont("H1",font,14):SetTextColor("H1",.9,.9,.7):SetSpacing("H1",5)
    :SetFont("H2",font,13):SetTextColor("H2",.9,.9,.7):SetSpacing("H2",4)
    :SetFont("H3",font,12):SetTextColor("H3",.9,.9,.7):SetSpacing("H3",3):SetIndentedWordWrap("H3",true)
    :un();
...
function UUI.Right.SetHTML(right, name)
    ...
    if not name then
        ...
        local text = "<HTML><BODY>"..format(UUI.Right.GetTitleFormat(), L["插件分类："]..CoreEncodeHTML(caption))..L["<P>　%s<br/><br/></P><P>　%s</P></BODY></HTML>"];
        right.pageDesc.html:SetText(format(text, CoreEncodeHTML(desc), L["插件数："]..num));
    else

        local info = U1GetAddonInfo(name);
        local title = U1GetAddonTitle(name, false)
        local name, _, notes, enabled, loadable, reason = GetAddOnInfo(name)
        local loaded = IsAddOnLoaded(name);
        ...
        local text = "<HTML><BODY>%s%s"..format(UUI.Right.GetTitleFormat(), L["插件介绍"])..L["<P>　%s<br/></P>%s</BODY></HTML>"];
        local author, modifier, changes = "", "", ""
        if info.author then
        ...
        page.html:SetText(format(text, author, modifier, desc, changes:gsub("<H3>%- </H3>","")));
    end
end
...
function UUI.OnShow(self)
    ...
    self.search.hint:Hide() --CoreUIShowOrHide(self.search.hint, U1DBG and U1DBG.hintSearch == nil)
...
function UUI:DB_LOADED()
    ...
    UIDropDownMenu_Initialize(UUI().setting.drop, UUI.Top.QuickSettingDropDownMenuInitialize, "MENU"); --在这就污染,在Core里处理
    UUI.DB_LOADED = nil
end
...
function UUI.CreateUI()
    ...
    :CreateAnimation("Alpha"):SetChange(-1):SetDuration(0.4):up()
    ...
    CoreRegisterEvent(UUI, "CURRENT_TAGS_UPDATED");
    CoreRegisterEvent(UUI, "CURRENT_ADDONS_UPDATED");
    CoreRegisterEvent(UUI, "ADDON_SELECTED");
    CoreRegisterEvent(UUI, "DB_LOADED");
    ...
    :CreateTexture():SetTexture(UUI.Tex"UI2-logo"):Size(128):TL(-14, 18):up()
...
function U1_CreateMinimapButton()
    ...
    local ldb = LibStub("LibDataBroker-1.1"):NewDataObject("!!!163UI!!!", {
        type = "launcher",
        label = L["网易有爱"],
    ...
    U1DB.minimapPos = U1DB.minimapPos or 226
    ...
    LibDBIcon10_U1MMB.tooltipTitle = L["网易有爱插件中心"]
    LibDBIcon10_U1MMB.tooltipText = function(self, tip)
        ...
        if self.__flash:IsVisible() then
            tip:AddLine(" ");
