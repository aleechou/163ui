...
--				local locName = strsplit (",", locName)
				...
				if strbyte (oStr, loc) == 32 then  -- Points
					...
					node.LocName = _TaxiName(node.LocName)		-- Localize it
					node.MapId = mapId
					node.WX = wx
					node.WY = wy
					tinsert (tdata, node)
...
--		local locName = strsplit (",", TaxiNodeName (n))
		...
		if TaxiNodeGetType (n) == "CURRENT" then
			...
			if NxData.DebugMap then
				...
				Nx.prt (L["Taxi current %s (%s)"], name or L["nil"], locName)
			end
		end
	end
end
...
--	map.TaxiNPCName = name
	...
	if NxData.DebugMap then
		Nx.prt (L["Taxi %s (%s) %.2f secs, node %d, %s %s"], name or L["nil"], map.TaxiName, tm, node, x or "?", y or "?")
	end
...
--				local destName = strsplit (",", TaxiNodeName (destNode))
				...
				if t == 0 then
					...
					if not t then
						...
						if NxData.DebugMap then
							Nx.prt (L[" No taxi data %s to %s"], srcName, destName)
						end
				...
				if NxData.DebugMap then
					Nx.prt (L[" #%s %s to %s, %s secs"], n, srcName, destName, t)
				end
...
--						Nx.prt ("DNPC %s", desc)
					...
					Nx.prt (L["Travel: missing dnpc %s %s"], destName, i)
				end
			end
		else
			Nx.prt (L["Travel: missing snpc %s %s"], srcName, i)
		end
	end
...
--								Nx.prt ("Ang %s %s = %s", ang1, ang2, ang)
								...
								local name = format (L["Connection: %s to %s"], Nx.MapIdToName[con.StartMapId], Nx.MapIdToName[con.EndMapId])
...
--		NxData.TaxiCap = {}
		...
		for n = 1, num do
			...
			Nx.prt (L["Taxi #%s %s, %s %f %f"], n, name, typ, x, y)
			tinsert (d, name)
		end
--[[
		local dest = 6
