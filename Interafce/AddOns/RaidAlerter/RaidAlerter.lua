...
OnlySelfCHAN = true, --XXX 163 false,				--仅本机
OnlySelfSCR = true, -- XXX 163 false,				--仅本屏
SoundAlert = true,					--音效
...
ToTCheck = true,					--目标提示
SpellIDAndCaster = false, -- xxx 163			--法编和源
OT_OnOff = false,					--OT通告
...
function RaidAlerter_CheckPvP(check)
	...
	-- new add 检测tank
	local RAL_checktanklistPlayer = "TANK:";
	...
	if RAL_PvPPlayerCount<RaidNum/2 then
		...
		for i = 1,GetNumGroupMembers() do
			...
			if subgroup <= RaidAlerter_SET.RAIDMODE_MAX_GROUP then
				...
				if RolesAssigned == "HEALER" then
					RAL_checkHEALERlistPlayer = RAL_checkHEALERlistPlayer..UnitName(unit).."(MP:"..UnitManaMax(unit)..").";
				end
				-- new add end
			end
		end
		...
		if check=="tip" then
			...
			if RAL_checkHEALERlistPlayer ~= "" then
				RAL.SendMSG("HEALER:"..RAL_checkHEALERlistPlayer,AlertChan,1);
			end
		end
		-- new add end
		if RAL_PvPPlayerCount>0 then
...
function RaidAlerter_Raid_PARTY_Combat_Alerter()
	...
	if eventtype == "SPELL_CAST_SUCCESS" then
		...
		-- 	return;
		-- end
		-- 114018,潜伏帷幕 ,31821 虔诚光环
		--武僧还魂术等
		if spellId == 31821 or spellId == 114018 or spellName == RAL_TEXT_SPELL_96 then
			TempUnit1 = RaidAlerter_NameToUnit(srcName);
			TempUnit2 = RaidAlerter_NameToUnit(dstName);
		...
		-- 英勇和嗜血及法师猎人宠物加速,神性牺牲,
		-- 集结呐喊 97462  武僧坦技能: 115203 盾 spellId 酒 106922 乌索克之力 740宁静 108280治疗之泉
		if spellId == 97462 or spellId == 62618 or spellId == 16190 or spellId == 32182 or spellId == 2825 or spellId == 80353 or spellId == 90355 or spellId == 70940 or spellId == 64843 or spellId == 64901 or spellId == 115203 or spellId == 115295 or spellId == 106922 or spellId == 740 or spellId == 108280 then
			TempUnit1 = RaidAlerter_GUIDToUnit(srcGUID);
			if TempUnit1 then
				if srcName~= nil then
					--仅通报坦
