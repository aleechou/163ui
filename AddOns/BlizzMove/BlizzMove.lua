...
local function hasUnitFrameAddOns()
    for _, addon in ipairs(unitFrameAddOns) do
        if IsAddOnLoaded(addon) or select(4, GetAddOnInfo(addon)) then
            return true
        end
    end
end
...
function BM_SetMoveHandler(frameToMove, handler)
    ...
    CoreHookScript(handler, "OnDragStart", OnDragStart)
...
local function OnEvent(self, event, arg1, arg2)
    ...
    if event == "PLAYER_ENTERING_WORLD" then
        ...
        BM_SetMoveHandler(WatchFrame,WatchFrameHeader)
        BM_SetMoveHandler(WatchFrame,WatchFrameCollapseExpandButton) --多个锚点的时候计算高度和宽度
        ...
        if not hasConflict and U1GetCfgValue and U1GetCfgValue("BlizzMove", "powerbar") then
            ...
            BM_SetMoveHandler(PaladinPowerBar)
            for i=1,3 do BM_SetMoveHandler(PaladinPowerBar, _G["PaladinPowerBarRune"..i]) end
            PaladinPowerBar:SetFrameStrata("LOW")
            BM_SetMoveHandler(RuneFrame)
            for i=1,6 do BM_SetMoveHandler(RuneFrame, _G["RuneButtonIndividual"..i]) end
            BM_SetMoveHandler(TotemFrame)
            for i=1,4 do BM_SetMoveHandler(TotemFrame, _G["TotemFrameTotem"..i]) end
            for i=1,5 do BM_SetMoveHandler(MonkHarmonyBar, _G["MonkHarmonyBarLightEnergy"..i]) end
            BM_SetMoveHandler(PriestBarFrame)
            BM_SetMoveHandler(WarlockPowerFrame, BurningEmbersBarFrame)
            BM_SetMoveHandler(WarlockPowerFrame, DemonicFuryBarFrame)
            BM_SetMoveHandler(WarlockPowerFrame, ShardBarFrame)
        end
        ...
        --BM_SetMoveHandlerWith(nil, "Blizzard_GlyphUI", function() BM_SetMoveHandler(SpellBookFrame, GlyphFrame) end); --无用了
        ...
        BM_SetMoveHandlerWith("GuildFrame", "Blizzard_GuildUI");
        ...
        if not hasConflict then BM_SetMoveHandler(PlayerPowerBarAlt) end
