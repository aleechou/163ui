...
local TradeskillInfo = LibStub("AceAddon-3.0"):NewAddon("TradeskillInfo", "AceConsole-3.0", "AceEvent-3.0", "AceHook-3.0", "LibShefkiTimer-1.0")
...
function TradeskillInfo:OnEnable()
	...
	self.OptionsPanel = LibStub("AceConfigDialog-3.0"):AddToBlizOptions("TradeSkillInfo", L["TradeSkill Info"])
...
function TradeskillInfo:GetCombineCost(id)
	...
	for _,c in ipairs(components) do
        if not c.cost and c.id and GetItemInfo(c.id) then
            c.cost = select(11, GetItemInfo(c.id)) or 0
        end
