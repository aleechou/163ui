...
function S:FormatDigit(digit)
	if (digit >= 1e8) then
		return L["yi"]:format(digit / 1e8);
	elseif (digit >= 1e4) then
		return L["wan"]:format(digit / 1e4);
	end
	return digit;
end
...
function BlinkHealth_SlashHandler(msg)
	...
	elseif (cmdtype == "hitoff") then
		...
		DEFAULT_CHAT_FRAME:AddMessage(BHT_1..BHT_2..BHT_3..BHT_4..BHT_5..BHT_6);
	end
end
...
function S:Delayclass()
	if IsLoggedIn()  then
		...
		self:ScheduleTimer("Delaydo",15);--延时15秒，dk符文在 PLAYER_ENTERING_WORLD事件前似乎不能获取信息
	end
end
function S:Delaydo()
	if (class == "DEATHKNIGHT") then		
