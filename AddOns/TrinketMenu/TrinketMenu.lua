...
function TrinketMenu.LoadDefaults()
	...
	TrinketMenuPerOptions = TrinketMenuPerOptions or {
		...
		XPos = GetScreenWidth()/2 + 250,					-- left edge of main window
		YPos = GetScreenHeight()/2 - 235,					-- top edge of main window
...
function TrinketMenu.BuildMenu()
	...
	if TrinketMenuOptions.KeepOpen~="ON" and not IsShiftKeyDown() and TrinketMenuOptions.MenuOnShift=="ON" then
		return
	end
...
function TrinketMenu.Initialize()
	...
	TrinketMenu.CreateTimer("UpdateWornTrinkets",TrinketMenu.UpdateWornTrinkets,.8)
	TrinketMenu.CreateTimer("DockingMenu",TrinketMenu.DockingMenu,.2,1)
	TrinketMenu.CreateTimer("MenuMouseover",TrinketMenu.MenuMouseover,.2,1)
	...
	TrinketMenu.CreateTimer("RedRange",TrinketMenu.RedRangeUpdate,.4,1)
...
function TrinketMenu.ResetSettings()
	StaticPopupDialogs["TRINKETMENURESET"] = {preferredIndex = 3,
...
function TrinketMenu.TimersFrame_OnUpdate(self, elapsed)
    ...
    if total_elapsed > 0.2 then
        local timers = TrinketMenu.Timers
        for i=#timers, 1, -1 do
            local name = timers[i]
            local timerPool = TrinketMenu.TimerPool[name]
            timerPool.elapsed = timerPool.elapsed - total_elapsed
