...
local function SaveAchievements(name, unit, isPlayer)
    ...
    for i, id in ipairs(TeamStats.db.map) do
        list[i] = GetAchieveOrStatById(id, TeamStats.stats[id], isPlayer)
    end
    player.stats = list
...
function TeamStats:VARIABLES_LOADED()
    TeamStats.db = TeamStatsDB or TeamStats:GetDefaultDB()
...
function TeamStats:ReMapData()
    ...
    for _, tab in ipairs(TABS) do
        for id, _ in pairs(tab.idName) do
