...
local additionalFilter = nil -- 额外过滤器 [string] (enabled|disabled)
local reloadList = {} --需要reload的情况
local initComplete --是否已经延迟加载完所有插件
--local variableLoaded --记录是否已经触发过variables_loaded事件
...
---------------------------------------------------------------]]
--多重依赖的情况，适应其他整合插件
local knownAddonPacks = { "duowan", "bigfoot", "mogu", "ace2", "ace3", "fish!!!" }
local deps = {};
local function getInitialAddonInfo()
    ...
    for i = 1, GetNumAddOns() do
        ...
        wipe(deps);
        deps[1], deps[2], deps[3], deps[4], deps[5], deps[6], deps[7], deps[8], deps[9] = GetAddOnDependencies(i)
        if (deps[1]) then
            for _, known in ipairs(knownAddonPacks) do
                for j, dep in ipairs(deps) do
                    if (dep:lower() == known) then
                        tremove(deps, j);
                        break;
                    end
                end
        ...
        local xcategories = UI163_USE_X_CATEGORIES and GetAddOnMetadata(i, "X-Category")
        ...
        --将X-OptionalDeps和剩余的Dependencies处理
        local xOptionalDeps = GetAddOnMetadata(i, "X-OptionalDeps")
        local optdeps
        if xOptionalDeps or #deps > 1 then
            optdeps = {}
            addonInfo[name:lower()].optdeps = optdeps
        end
        if (#deps > 1) then
        ...
        if(xOptionalDeps)then
            local xopt = {strsplit(",", xOptionalDeps)}
            for j=1, #xopt do
                local opt = strtrim(xopt[j]):lower()
                if not tContains(optdeps, opt) then tinsert(optdeps, opt) end
            end
        end

        --关闭插件, 在VARIABLE_LOADED事件时再打开
        --DisableAddOn(name); --改为在ConfigsLoaded里面调用
    end
...
function U1ConfigsLoaded()
    ...
    for k,v in pairs(addonInfo) do
        if not v.registered and v.xcategories then
            U1RegisterAddon(v.name, { alwaysRegister = true, tags = {strsplit(",", v.xcategories:gsub(",[ ]+", ","))}})
        end
        v.xcategories = nil
    end
    for k,v in pairs(addonInfo) do
        ...
        if v.lod and not v.parent and not v.nolodbutton then
            tinsert(v, 1, {text=L["强制加载"], enableOnNotLoad = 1, disableOnLoad = 1, tip=L["说明`本插件会在满足条件时自动加载，如果现在就要加载请点击此按钮` `|cffff0000注意：有可能会出错|r"], callback=function()
                if not IsAddOnLoaded(k) then
                    local loaded, reason
                    if U1IsNeteaseVendor(k) then loaded, reason = U1LoadAddOn(k) else loaded, reason = LoadAddOn(k) end
                    U1OutputAddonLoaded(k, loaded, reason);
                    if loaded then UUI.Right.ADDON_SELECTED() UUI.Center.Refresh() end
                end
            end})
        end
        local pinfo = addonInfo[v.parent]
        ...
        if v.dummy then
            v.defaultEnable = v.defaultEnable or 1 --dummy默认启用
        ...
        if(v.iconText)then
            tinsert(addonInfo[k], 1, {
                type="button",
                text=v.iconText,
                tipLines=v.iconTip and {strsplit("`", (v.iconTip:gsub("%$title", v.title)))},
                callback = v.iconFunc,
            });
        end
    end
    wipe(parentTags); parentTags = nil
...
do
    ...
    function U1LoadDBValue(cfg)
        ...
        if(v == nil and cfg.default) then
            v = load_value(cfg.default)
        end
...
function U1SaveDBValue(cfg, value)
    ...
    local oldvalue = U1EncodeNIL(U1LoadDBValue(cfg)) --读取default
    if cfg._path:find("^"..strlower(_)) and U1DB.configs[cfg._path]~=nil then U1DB.configs[cfg._path] = nil end --触发newindex
...
--path可以省略,直接写绝对路径
function U1CfgCallBackByPath(path, forceValue, loading)
    local _, cfg = U1GetCfgValue(path)
    if(cfg)then
        U1CfgCallBack(cfg, forceValue, loading)
    end
end
...
function U1CfgCallSub(cfg, sub, v)
    U1CfgCallBack(U1CfgFindChild(cfg, sub), not not v and nil);
end
...
function U1UpdateTags(onlyAffectLoaded, addonName)
    ...
    for k, v in pairs(U1.TAGS) do
        if (type(v)=="table" and v.filter) then
            --计数清0
            if not tagInfo[k] then registerTag(k) end
            tagInfo[k].num = 0;
...
local comparatorLastClick = function(v1, v2)
    local o1 = tagInfo[v1].order or math.huge;
    local o2 = tagInfo[v2].order or math.huge;
    if (o1 == o2) then
        --local t1 = db.tags[v1] and db.tags[v1][1] or 0;
        --local t2 = db.tags[v2] and db.tags[v2][1] or 0;
        local t1,t2 = 0, 0
        if t1 == t2 then
            return v1 < v2
        else
...
function U1SortTagByLastClick()
    table.sort(currTags, comparatorLastClick);
    CoreFireEvent("CURRENT_TAGS_UPDATED");
end

function U1SearchTag(text)
    local pattern = nocase(escape(text));
    wipe(currTags);
    for k, v in pairs(tagInfo) do
...
function U1SelectTag(tag, keepSelectedAddon)
    --db.tag = {count, lastTime}
    db.selectedTag = tag;
    --db.tags[tag] = db.tags[tag] or {0};
    --db.tags[tag][1] = db.tags[tag][1] + 1;
    --db.tags[tag][2] = GetTime();
    U1SelectAddon(keepSelectedAddon and db.selectedAddon or nil); --这里如果固定选择nil, 则切换标签时会清除, 但按ESC和RELOAD之后就会有问题 U1GetSelectedAddon()
...
function U1GetTagInfoByName(name)
    ...
    if not info then name = "NETEASE" info = tagInfo[name] end
    info.caption = info.caption or info.text .. ((name=="LOADED" or name=="NLOADED") and "(" .. info.num .. ")" or "");
    local desc = L["TAG_DESC_" .. name] or info.text
...
function U1RegisterAddon(name, infoReg)
    ...
    if not infoRaw and not infoReg.dummy then parentTags[name:lower()] = infoReg.tags return end --未安装的插件，没必要保留空目录，可以节省内存
    ...
    infoReg.order = order;
    order = order + 1;
    infoReg.registered = true; --标记为注册的插件
    ...
    if (infoRaw) then
        if(not infoRaw.lod) then infoReg.load = infoReg.load or (UI163_USER_MODE and not infoRaw.vendor and "NORMAL" or "LATER") end --设置非LOD的默认的加载方式为LATER
        infoRaw.lod = (infoRaw.lod and infoReg.load==nil) or (infoReg.load=="DEMAND"); --reg.load可以覆盖LoadOnDemand，因为大脚魔盒之类的一些插件都加了标签
        ...
        for k, v in pairs(infoRaw) do
            ...
            if k=="optdeps" then
                --合并两个optdeps
                if infoReg[k] then
                    for _, opt in ipairs(infoReg[k]) do
                        local optlower = opt:lower()
                        if not tContains(v, optlower) then
                            tinsert(v, optlower)
                        end
                    end
                end
    ...
    if infoReg.optdeps then
        for i, opt in ipairs(infoReg.optdeps) do infoReg.optdeps[i]=opt:lower() end
        for i = #infoReg.optdeps, 1, -1 do if not addonInfo[infoReg.optdeps[i]] then tremove(infoReg.optdeps, i) end end --减少内存
    end
    ...
    if (infoReg.parent and not infoReg.tags and not infoRaw.parent and parentTags[infoReg.parent:lower()]) then
        infoReg.tags = parentTags[infoReg.parent:lower()]
        infoReg.parent = nil
    end

    --处理标签,只处理没有依赖的
    if (infoReg.parent == nil) then
        if(not infoReg.hide) then
            infoReg.tags = infoReg.tags or (infoReg.xcategories and {strsplit(",", infoReg.xcategories:gsub(",[ ]+", ","))}) or {};
            for _, v in ipairs(infoReg.tags) do
                --转换为hash，用于快速检索.
                if v == "CLASS" then infoReg._classAddon = true end --标记插件是职业相关，如果不是本职业则隐藏
                if true or infoReg.vendor then --只处理标准的。
                    infoReg.tags[v] = true;
                    registerTag(v);
                end
            end
        ...
        infoReg.parent = infoReg.parent:lower();
    end
...
function U1ChangeTags(name, tags, add)
    ...
    if info and info.registered then
        if not add then
...
function U1IsAddonRegistered(name)
    name = name:lower();
    local info = addonInfo[name]
    return info and info.registered and true, info and (info.vendor or info.dummy)
end
...
function U1GetAddonTitle(name)
    local info = U1GetAddonInfo(name);
    if(info.dummy)then return info.title end --不显示原名
...
--[[------------------------------------------------------------
后加载插件的功能, 模拟事件，TODO: 其他的事件，或者其他的Frame类型
---------------------------------------------------------------]]
...
--UnregisterEvent是在Simulate里用IsEventRegistered来等价实现
hooksecurefunc(getmetatable(UIParent).__index, "RegisterEvent", captureHook);
hooksecurefunc(getmetatable(CreateFrame("Button")).__index, "RegisterEvent", captureHook);
hooksecurefunc(getmetatable(GameTooltip).__index, "RegisterEvent", captureHook);
...
local function initPageConfigs()
    for k, page in pairs(addonInfo) do
        for i=1,#page do
            deepInit(nil, page[i], k);
        end
    end
end
...
function U1:ADDON_LOADED(event, name)
    if(name==_)then
        ...
        U1.db = db;
        U1DBG = U1DBG or {}
        setmetatable(db.configs, {__newindex=function(t, k, v) rawset(t, k, v) if(_G[k])then _G[k](v) end end}) --设置了值就会执行对应的函数，代码保护，声音设置的
        if db.configs[cfgName]==1 then toggle(1) end --如果不是1则不执行hook
        ...
        for name,info in pairs(addonInfo) do
            ...
            if not db.addons[name] then
                local enabled = false
                if info.parent and not info.defaultEnable then --子插件
            ...
            if(db.addons[name]==1 and not info.dummy) then
                ...
                if info.lod or info.protected then
        ...
        --处理既是NORMAL，又需要deps的
        for k, v in U1IterateAllAddons() do
            if v.load=="NORMAL" and v.deps then
                local tmp = v;
                while(true) do
    ...
    while (used < 0.2) do
        ...
        if(not name) then
            if not secure and #addonToLoadSecure>0 and not needSecureLoadAnnounced then
                U1Message(L["因为5.0系统的改动，战斗中无法加载插件，战斗结束后会继续。如果不想使用延迟加载，请到控制台'有爱设置'中，关闭'进入世界后再加载插件'的选项。"])
        ...
        if(not IsAddOnLoaded(name)) then
            local info = U1GetAddonInfo(name)
            if InCombatLockdown() then --and info.secure --5.0 script ran too long
                ...
                if not secureLoadAnnounced and secure then
                    ...
                    if speed == 0 then speed = 100 elseif speed==100 then speed = 0 else speed = 0.4/speed end
                    used = used + speed;
                end
            end
        end
...
function U1:PLAYER_ENTERING_WORLD(event)
    ...
    loadTimer = CoreScheduleTimer(true, 0.3, loadAddon, false);
    loadTimerSecure = CoreScheduleTimer(true, 0.3, loadAddon, true);
...
do
    ...
    local hookStopMovingOrSizing = function(self)
        ...
        pos[1], pos[2], pos[3], pos[4] = self:GetLeft(), self:GetTop(), self:GetWidth(), self:GetHeight()
    ...
    function U1FramePosRestore(name)
        ...
        if _G[name] and pos and #pos > 1 then
            ...
            if not pos[7] or not _G[pos[7]] then
                _G[name]:SetPoint("TOPLEFT", "UIParent", "BOTTOMLEFT", pos[1], pos[2]);
                if _G[name]:IsMovable() then
                    --系统自动计算位置，缩放界面保持原位置
                    _G[name]:StartMoving();
                    _G[name]:StopMovingOrSizing();
                end
            else
                _G[name]:SetPoint(pos[6],pos[7],pos[8],pos[9],pos[10])
            end
        end
    end
end
