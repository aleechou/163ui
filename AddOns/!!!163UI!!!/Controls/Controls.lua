...
WW:Font("CtlFontNormalSmall", ChatFontNormal, 13):SetTextColor(0.81, 0.65, 0.48):SetShadowOffset(1,-1):un();
WW:Font("CtlOptionFontNormalSmall", ChatFontNormal, 12):SetTextColor(0.40, 0.33, 0.24):un();
WW:Font3("CtlFontSmall", ChatFontNormal, 13, {{0.81, 0.65, 0.48},{1, .96, .63},{.5,.5,.5}}, nil, 1, -1);
WW:Font3("CfgButtonFont", ChatFontNormal, 12, {{0.81, 0.65, 0.48},{1, .96, .63},{.5,.5,.5}}, nil, 1, -1);
...
function CtlRegularTip(self, cfg)
    if not cfg.tipLines and cfg.tip then
        ...
        cfg.tip = nil
        if cfg.reload then tinsert(cfg.tipLines, " "); tinsert(cfg.tipLines, L["|cffff0000需要重新加载界面|r"]); end
    end
    if cfg.reload and not cfg.tipLines then cfg.tipLines = {L["注意"], L["|cffff0000需要重新加载界面|r"]} end
