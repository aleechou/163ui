...
local function getJPackItem(bag,slot,isGB)
    ...
    if link:find("battlepet:") then link = select(2, GetItemInfo(6948)) end
    local item={}
    item.slotId=slot
...
local function getPerffix(item)
    ...
    elseif(IsEquippableItem(item.name) and item.type~=L.TYPE_BAG and item.subType~=L.TYPE_FISHWEAPON) and item.subType~=L.TYPE_MISC then
        if(item.rarity <= 1 ) then --or (item.level<UnitLevel('player')*0.9)then
            return '02'..s
        end
--[[
...
local function groupBags()
    ...
    local ignored_bank = JPACK_IGNORE_BAGS_NO_BANK or dummy_table
    local bagTypes={}
    bagTypes[L.TYPE_BAG]={}
    if not ignored[0] and not ignored_bank[0] then
        bagTypes[L.TYPE_BAG][1]=0
    end
    for i=1,4 do
        if not ignored[i] and not ignored_bank[i] then
    ...
    if(JPack.bankOpened)then
        ...
        if not ignored[-1] and not ignored_bank[-1] then
...
function JPack:ADDON_LOADED(event, addon)
    ...
    _G.JPackDB = _G.JPackDB or {}
    JPackDB = setmetatable(_G.JPackDB, {__index = defaults})
